    ...
    render() {
        ...
        return (
            <div css={sectionStyle(this.props)}>
                <h6 css={sectionHeaderStyle(this.props)}>Elementos compartidos</h6>
                <div css={sectionContentStyle(this.props)} data-id="sharedmedia">
                    <div css={mediaBtnStyle()}>
                        <span css={buttonStyle(this.state, "image")} onClick={() => this.mediaClickHandler("image")}>Imágenes</span>
                        <span css={buttonStyle(this.state, "video")} onClick={() => this.mediaClickHandler("video")}>Videos</span>
                        <span css={buttonStyle(this.state, "file")} onClick={() => this.mediaClickHandler("file")}>Documentos</span>
                    </div>
                    <div css={mediaItemStyle()}
                    ref={el => this.messageContainer = el}
                    onScroll={this.handleScroll}>{(messageList.length) ? messageList : "No se encontraron registros."}
                    </div>
                </div>
            </div>
        );

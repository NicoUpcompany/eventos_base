...
class CometChatUserDetail extends React.Component {
    ...
    render () {
        ...
        if(this.props.item.blockedByMe) {
            blockUserText = (
                <div css={itemLinkStyle(1, this.props)} onClick={() => this.props.actionGenerated("unblockUser")}>Desbloquear usuario</div>
            );
        } else {
            blockUserText = (
                <div css={itemLinkStyle(1, this.props)} onClick={() => this.props.actionGenerated("blockUser")}>Bloquear usuario</div>
            );
        }

...
class MessageThread extends React.Component {
        ...
        <div css={parentMessageContainerStyle(message, this.props)}>                                
          ...
          <div>
            <span css={messageTimestampStyle()}>
              {new Date(message.sentAt * 1000).toLocaleTimeString('es-CL', { hour: 'numeric', minute: 'numeric', hour12: true })}
            </span>
          </div>
        </div>                            
      );
    ...
    render() {
      ...
      if(this.state.replyCount) {
        ...
        const replyText = (replyCount === 1) ? `${replyCount} respuesta` : `${replyCount} respuestas`;
          ...
          <div css={headerStyle(this.props)}>
            <div css={headerWrapperStyle()}>    
              <div css={headerDetailStyle()}>
                <h6 css={headerTitleStyle()}>Respuestas</h6>

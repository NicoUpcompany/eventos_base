...
class CometChatAddMembers extends React.Component {
    ...
    searchUsers = (e) => {
        ...
        // this.AddMembersManager = new CometChat.UsersRequestBuilder().setSearchKeyword(val).build();
        //     this.setState({ userlist: [], membersToAdd: [], membersToRemove: [] }, () => this.getUsers())
        // }, 500);
        ...
        new CometChatManager().getLoggedInUser().then((user) => {
            ...
            this.AddMembersManager.fetchNextUsers().then((userList) => {
                ...
                if (userList.length === 0) {
                    this.decoratorMessage = "No hay usuarios";
                }
        ...
        return (
            <React.Fragment>
                ...
                <div css={modalWrapperStyle(this.props)}>
                    <span css={modalCloseStyle(clearIcon)} onClick={this.props.close} title="Cerrar"></span>
                    <div css={modalBodyCtyle()}>
                        <table css={modalTableStyle()}>
                            <caption css={tableCaptionStyle()}>Contactos</caption>
                            ...
                            <tfoot css={tableFootStyle(this.props)}>
                                <tr>
                                    <td colSpan="2"><button onClick={this.updateMembers}>Agregar</button></td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>

...
class MessageComposer extends React.PureComponent {
  ...
  render() {
    ...
    let docs = (
      <span title="Seleccionar Archivo" css={fileItemStyle(this.props, docIcon)} onClick={() => { this.openFileDialogue("file") }}>
    ...
    let avp = (
      <React.Fragment>
        <span title="Seleccionar Video" css={fileItemStyle(this.props, videoIcon)} onClick={() => { this.openFileDialogue("video") }}>
          <input onChange={this.onVideoChange} accept="video/*" type="file" ref={this.videoUploaderRef} />
        </span>
        <span title="Seleccionar Audio" css={fileItemStyle(this.props, audioIcon)} onClick={() => { this.openFileDialogue("audio") }}>
          <input onChange={this.onAudioChange} accept="audio/*" type="file" ref={this.audioUploaderRef} />
        </span>
        <span title="Seleccionar Imagen" css={fileItemStyle(this.props, imageIcon)} onClick={() => { this.openFileDialogue("image") }}>
    ...
    let emojiBtn = (
      ...
      onClick={this.toggleEmojiPicker}><img src={insertEmoticon} alt="Insertar emoticon" /></div>
    );
          ...
          <div tabIndex="-1" css={inputInnerStyle(this.props)}>
            ...
            <div css={inputStickyStyle(this.props)}>
              ...
              <div css={stickyButtonStyle()} ref={node => {this.node = node;}}>
                {emojiPicker}
                {emojiBtn}
                <div title="Enviar mensaje" css={sendButtonStyle()} onClick={this.sendTextMessage}><img src={sendBlue} alt="Enviar mensaje" /></div>
              </div>
            </div>
          </div>
        </div>
